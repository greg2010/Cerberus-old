...
trait UserService {
  def createUser(email: String, password: Option[String], credentials: Credentials): Future[UserMini]
...
class UserController(permissionController: => PermissionController,
  ...
  override def createUser(email: String,
                          ...
                          credentials: Credentials): Future[UserMini] = {
  ...
  override def verifyUser(nameOrEmail: String, providedPassword: String): Future[UserMini] = {
    ...
    val f = dbAgent.run(query.result).map {
      _.headOption.map(User.apply) match {
