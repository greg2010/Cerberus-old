...
import org.red.cerberus.jobs.quartz.{TeamspeakDaemonJob, TeamspeakJob, UserJob}
...
class ScheduleController(config: Config, userController: => UserController, teamspeakController: => TeamspeakController)
                        (implicit dbAgent: JdbcBackend.Database, ec: ExecutionContext)
  extends ScheduleService with LazyLogging {

  private val daemon = new ScheduleDaemon(this, config, userController, teamspeakController)
